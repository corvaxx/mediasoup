...
class Producer extends EnhancedEventEmitter_1.EnhancedEventEmitter {
     ...
     * Set translate mode
     */
    async setTranslateMode(translateMode) {
        logger.debug('setTranslateMode()');
        const reqData = { translateMode };
        await this._channel.request('producer.setTranslateMode', this._internal, reqData);
        this._translateMode = translateMode;
    }
    /**
     * Start master mode
     */
    async startMasterMode(ssrc, width, height) {
        logger.debug('startMasterMode()');
        const reqData = { ssrc: ssrc, width: width, height: height };
        await this._channel.request('producer.startMasterMode', this._internal, reqData);
    }
    /**
     * Attach produser to producer as slave
     */
    async attachSlave(slaveId) {
        logger.debug('setTranslateMode()');
        const reqData = { slaveId };
        await this._channel.request('producer.attachSlave', this._internal, reqData);
    }
    /**
