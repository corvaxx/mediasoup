...
exports.Mixer = exports.MIXER_RENDER_MODE = void 0;
...
var MIXER_RENDER_MODE;
(function (MIXER_RENDER_MODE) {
    MIXER_RENDER_MODE["SCALE"] = "scale";
    MIXER_RENDER_MODE["CROP"] = "crop";
    MIXER_RENDER_MODE["PAD"] = "pad";
})(MIXER_RENDER_MODE = exports.MIXER_RENDER_MODE || (exports.MIXER_RENDER_MODE = {}));
class Mixer extends EnhancedEventEmitter_1.EnhancedEventEmitter {
    ...
    async update(producerId, options) {
        logger.debug('update()');
        if (this._producers.size === 0)
            throw new TypeError('no mixer producers');
        ...
        const status = await this._channel.request('mixer.update', internal, reqData);
    }
    async remove(producerId) {
        logger.debug('remove()');
        if (this._producers.size === 0)
            throw new TypeError('no mixer producers');
        if (this._producers.has(producerId))
        ...
        const internal = { ...this._internal, mixerProducerId: mixerProducer.id, producerId: producerId };
        const reqData = {};
        const status = await this._channel.request('mixer.remove', internal, reqData);
    }
}
exports.Mixer = Mixer;
