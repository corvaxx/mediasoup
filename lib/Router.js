...
class Router extends EnhancedEventEmitter_1.EnhancedEventEmitter {
     ...
     * Create an Mixer.
     */
    async createMixer(appData = {}) {
        logger.debug('createMixer()');
        if (appData && typeof appData !== 'object')
            throw new TypeError('if given, appData must be an object');
        const internal = { ...this._internal, mixerId: uuid_1.v4() };
        const reqData = {};
        await this._channel.request('router.createMixer', internal, reqData);
        const mixer = new Mixer_1.Mixer({
            internal,
            channel: this._channel,
            payloadChannel: this._payloadChannel,
            appData,
        ...
        mixer.on('@newproducer', (producer) => this._producers.set(producer.id, producer));
        mixer.on('@producerclose', (producer) => this._producers.delete(producer.id));
        return mixer;
    }
    /**
